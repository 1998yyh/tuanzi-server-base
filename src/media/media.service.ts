...
export class MediaService {
  ...
  async saveUpload(userId: string, file: Express.Multer.File): Promise<MediaFileView> {
    ...
    const dimensions = kind === MediaKind.IMAGE ? await this.readImageSize(file.path) : null;
  ...
  async saveBuffer(
    ...
    const ext = EXT_BY_MIME[options.mimeType] ?? '.bin';
  ...
  async findByIdsForUser(ids: string[], userId: string): Promise<MediaFile[]> {
    ...
    const medias = await this.mediaRepo.findByIds(ids);
  ...
  private readImageSizeFromBuffer(buffer: Buffer): { width: number; height: number } | null {
    try {
